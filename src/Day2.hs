...
executeCommands doCommand =
  show
    . multiplyLocation
    . foldl doCommand Location {depth = 0, horizontal = 0, aim = 0}
    . parseCommands
...
doCommandWithAim loc (Forward x) =
  loc
    { horizontal = horizontal loc + x,
      depth = depth loc + aim loc * x
    }
